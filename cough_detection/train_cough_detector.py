...
def add_librispeech_to_dataset(X, files_used, load_type, full_path, ratio=0.5, cepstrum_dimension=100, time = 5):
    ...
    if load_type == "job":
        ...
        print("wron specification of load type")
        files_libri, libri = (None, None)
        exit()

    if full_path:
        files_libri = ["/store/datasets/aux/all_librispeech_links/" + f for f in files_libri]
...
def main(make_chunks=False, pre_train=False, full_paths=False, multi_input = False, add_librispeech=False):
    ...
    # Make user write a comment to be saved
    print("Write a experiment description to be saved with the results")
    comment = str(input())
    with open(save_path + "user_comment.txt", "w") as text_file:
        text_file.write(comment)

    # Parameters used:
    epochs = variables['epochs']        # Number of repetitions of the training
    patience = variables['patience']    # Epochs waited without improvements before early stopping the training
    monitor = variables['monitor']      # Monitored value expected to improve
    lr = variables['lr']                # Learning rate
    opt = variables['opt']              # Optimizer
    steps = variables['steps']          # learning steps
    ...
    if pre_train:
        ...
        with mirrored_strategy.scope():
            path_libri = train_basemodel('Librispeech', save_path, model_type="ResNet50",
                                         monitor='val_loss', epochs=epochs, lr=lr,
                                         save_name="RN50_t5_c100", batch_size=50, patience=10)

            path_sentiment = train_basemodel('Sentiment', SAVE_DIR + EXP + '/', model_type="ResNet50",
                                             monitor='val_loss', epochs=epochs, lr=0.001,
                                             save_name="RN50-new_t5_c100", batch_size=10, patience=50)
    ...
    if load_type == 'import':
        ...
        # If we want files_used to contain the full path:
        if full_paths:
        ...
        print("calculate sft for language: ", language)
        X_pos, X_neg, files_used_p, files_used_n = import_language_PN_STFT(language, 512)
        print('files positives:', len(files_used_p))
        print('files negatives:', len(files_used_n))
    ...
    # Previous train/val split method
    # X = np.concatenate((X_pos, X_neg), axis=0)
    # Y = np.concatenate((np.ones(len(X_pos)), np.zeros(len(X_neg))))
    # X_train, y_train, X_test, y_test, X_val, y_val = split_dataset(X, Y, test_size, val_split)
